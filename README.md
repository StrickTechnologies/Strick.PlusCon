...
If an option in the a menu's `Options` collection contains a key in its `Keys` collection that is also 
contained in the menu's `ExitKeys` collection, the menu option takes precedent.
If two (or more) options in the a menu's `Options` collection contain the same key a key in their `Keys` collections, 
the option with the lowest index within the collection will take precedent.
...
An individual option can override the menu-level style via its `Style` property.
